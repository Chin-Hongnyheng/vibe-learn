...
-- ---- users -------------------------------------------------
-- Synced from Clerk by the backend webhook. Never written by the browser.
...
-- ---- progress ----------------------------------------------
-- Written only through the Express backend.
...
-- Public read (catalog content — safe to expose with anon key)
...
-- Auth-gated reads — each learner sees only their own rows
...
create policy "users read own row"           on users         for select using (auth.uid()::text = id);
create policy "users read own progress"      on progress      for select using (auth.uid()::text = user_id);
create policy "users read own enrollments"   on enrollments   for select using (auth.uid()::text = user_id);
create policy "users read own notifications" on notifications for select using (auth.uid()::text = user_id);


-- ===========================================================
-- SECTION 3: SEED — INSTRUCTORS
-- ===========================================================
...
-- ===========================================================
-- SECTION 4: SEED — COURSES  (10 courses from mock-data.ts)
-- ===========================================================

with inst as (select id, name from instructors)
insert into courses
...
values
  ...
  (
    ...
    'Intermediate', 1104, 4.9, 1280, 2100, 0.00, true, true, 'English',
    '[
      {"icon":"layers",   "title":"App Router Foundations",     "description":"Master the App Router, layouts, loading states, and nested routing."},
      {"icon":"database", "title":"Data Fetching & Caching",    "description":"Fetch data efficiently and leverage caching for better performance."},
      {"icon":"gauge",    "title":"Performance Optimization",   "description":"Optimize rendering, assets, and bundle size for faster apps."},
      {"icon":"cloud",    "title":"Deployment & Scaling",       "description":"Deploy with confidence and scale your Next.js applications."}
    ]'::jsonb,
  ...
  -- 3. UI/UX Design Fundamentals with Figma
  (
  ...
  -- 4. iOS App Development with Swift & SwiftUI
  (
  ...
  -- 5. Digital Marketing Masterclass
  (
    ...
    'Beginner', 720, 4.5, 3910, 22100, 0.00, true, true, 'English',
    null,
  ...
  (
    ...
    'Beginner', 480, 4.7, 1870, 10500, 0.00, true, false, 'English',
    null,
    (select id from inst where name = 'Marco Rossi'),
    '2024-07-08T14:00:00Z'
  ),
  -- 8. Full-Stack Web Development with Node.js
  (
  ...
  -- 9. Personal Finance & Investing 101
  (
  ...
  -- 10. Graphic Design with Adobe Illustrator
  (
...
-- ===========================================================
-- SECTION 5: SEED — MODULES  (all courses)
-- ===========================================================

-- Course 1 — Next.js for Production (rich modules with description + duration)
...
-- ===========================================================
-- DONE — verify with:
--   select count(*) from instructors;   -- should be 10
--   select count(*) from courses;       -- should be 10
--   select count(*) from modules;       -- should be 83
-- ===========================================================
